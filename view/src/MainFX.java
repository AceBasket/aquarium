 ...
 * }
 * 
 * public ImageView createFish(Coordinates position, int fishWidth, int
 * fishHeight) {
 ...
 * // Assuming you have a target coordinate (targetX, targetY) and a duration in
 * // seconds
 * 
 * Fish fish = fishes.get(i);
 * Coordinates destination = fish.getFirstDestination();
 * Coordinates destinationP = new Coordinates((int)
 * pourcentageToPixelX(destination.getX()),
 * (int) pourcentageToPixelY(destination.getY()));
 * double duration = (double) fish.getTimeToGetToFirstDestination() -
 ...
 * if (duration != 0) {
 * double velocityX = distanceX / (duration); // Divide by the number of frames
 * per second (e.g.,
 * // 60)
 ...
 * // if ((velocityX > 0 && fishX >= destinationP.getX()) || (velocityX < 0 &&
 * // fishX <= destinationP.getX())
 * // || (velocityY > 0 && fishY >= destinationP.getY()) || (velocityY < 0 &&
 * fishY
 * // <= destinationP.getY())) {
 ...
 * // if (fishIV.getBoundsInParent().getMaxX() < 0 ||
 * // fishIV.getBoundsInParent().getMinX() > VIEW_WIDTH ||
 * // fishIV.getBoundsInParent().getMaxY() < 0 ||
 * // fishIV.getBoundsInParent().getMinY() > VIEW_HEIGHT) {
 ...
 * ConcurrentLinkedQueue<ParserResult> receivedQueue = new
 * ConcurrentLinkedQueue<ParserResult>();
 * ConcurrentLinkedQueue<String> sendQueue = new
 * ConcurrentLinkedQueue<String>();
 * Runnable readFromServerThread = new ReadFromServerThread(view, receivedQueue,
 * sendQueue);
 * Runnable serverThread = new ServerThread(view, aquarium, receivedQueue,
 * sendQueue);
 * Runnable promptThread = new PromptThread(view, aquarium, receivedQueue,
 * sendQueue);
 ...
 * Platform.runLater(() -> {
 * ImageView fishIV = createFish(fish.getPosition(), fish.getLength(),
 * fish.getHeight());
