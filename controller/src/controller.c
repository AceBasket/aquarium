        ...
        for (int num_view = 0; num_view < MAX_VIEWS; num_view++) {
            if (FD_ISSET(views_socket_fd[num_view], &read_fds)) {
                ...
                case HELLO:
                    ...
                    hello_handler(parser, views_socket_fd[num_view], aquarium);
                    break;
                case GETFISHES:
                    fprintf(log, "Get fishes from view %d\n", num_view);
                    get_fishes_handler(parser, views_socket_fd[num_view], aquarium);
                    break;
                case GFCONTINUOUSLY:
                    fprintf(log, "Get fishes continuously from view %d\n", num_view);
                    get_fishes_continuously_handler(parser, views_socket_fd[num_view], aquarium);
                    break;
                case LS:
                    fprintf(log, "List fishes from view %d\n", num_view);
                    ls_handler(parser, views_socket_fd[num_view], aquarium);
                    break;
                case PING:
                    fprintf(log, "Ping from view %d\n", num_view);
                    ping_handler(parser, views_socket_fd[num_view], aquarium);
                    break;
                case ADDFISH:
                    fprintf(log, "Add fish from view %d\n", num_view);
                    add_fish_handler(parser, views_socket_fd[num_view], aquarium);
                    break;
                case DELFISH:
                    fprintf(log, "Delete fish from view %d\n", num_view);
                    del_fish_handler(parser, views_socket_fd[num_view], aquarium);
                    break;
                case STARTFISH:
                    fprintf(log, "Start fish (%s) from view %d\n", parser->arguments[1], num_view);
                    start_fish_handler(parser, views_socket_fd[num_view], aquarium);
                    break;
                case LOG:
                    fprintf(log, "LOGOUT out from view %d\n", num_view);
                    log_out_handler(parser, views_socket_fd[num_view], aquarium);
                    break;
                case STATUS:
                    fprintf(log, "Status from view %d\n", num_view);
                    status_handler(parser, views_socket_fd[num_view], aquarium);
                    break;
                case UNKNOWN:
...
void *thread_prompt() {
    ...
    while (1) {
        ...
        case ADD_VIEW:
            ...
            // coord.x = atoi(parser->arguments[3]);
            // coord.y = atoi(parser->arguments[4]);
            // view = create_view(parser->arguments[2], coord, atoi(parser->arguments[5]), atoi(parser->arguments[6]));
        ...
        case DEL_VIEW:
            ...
            // remove_view(aquarium, get_view(aquarium, parser->arguments[2]));
